    ...
    for z in ["Base"#=, "GMP", "GMP-LowCH4", "GMP-HighCH4"=#]
        ...
        for TP in ["NoTPs"#=, "TPs",=#"NoOMH"]
            ...
            for persistence in ["high"#=, "default"=#]
                ...
                CSV.write("../results/sccs-$x$z-$y-$TP-$persistence-Replication.csv", sccresults)#=REMOVE SUFFIX AFTERWARDS=#
                if calc_nationals
                    CSV.write("../results/natscch4s-$x$z-$y-$TP-$persistence-Replication.csv", allscch4results)#=REMOVE  SUFFIX AFTERWARDS=#
                end
            end
    end
end
